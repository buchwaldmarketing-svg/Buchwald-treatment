            ...
            {pts.map(t => { const sc = statusColors[t.status]||statusColors.presented; return <div key={t.id} style={{ background:"#f7f9fb", borderRadius:10, padding:"12px 14px", marginBottom:10 }}>
              ...
              {t.type === "Treatment Plan" && t.status !== "paid" ? (<>
                <div style={{ display:"flex", gap:6, marginBottom:6 }}>
                  ...
                  }} style={{ flex:1, padding:"8px 12px", background:"white", color:BLUE, border:`1.5px solid ${BLUE}`, borderRadius:8, fontSize:12, fontWeight:700, cursor:"pointer" }}>{"\u{1F4CB}"} View Plan</button>
                  <button onClick={() => {
                    ...
                    setSelectedPatient(null);
                    setAppMode("receipt");
                  }} style={{ flex:1, padding:"8px 12px", background:"white", color:GREEN, border:`1.5px solid ${GREEN}`, borderRadius:8, fontSize:12, fontWeight:700, cursor:"pointer" }}>{"\u{1F9FE}"} Receipt</button>
                </div>
              </>) : (
                <button onClick={() => {
                  setRcptName(`${selectedPatient.first_name} ${selectedPatient.last_name}`);
  ...
  if (appMode === "warranty" && wPreview) {
    return (<div style={{ background:"#f0f0f0", minHeight:"100vh", fontFamily:"Arial, sans-serif" }}>
      ...
      <div id="w-pdf"><div className="print-page">
        ...
        <div style={{ paddingLeft:14, fontSize:10, lineHeight:1.65, marginBottom:6 }}><div style={{ marginBottom:4 }}>1. <b>Regular Cleanings</b> every 6-7 months (or Perio Maintenance every 3-4 months for patients with bone loss) — the foundation of long-lasting restorations</div><div style={{ marginBottom:4 }}>2. <b>Custom Nightguard</b> (starting at $400 depending on insurance) — worn regularly, this protects your crowns, implants, and fillings from grinding and clenching, one of the top causes of restoration failure</div><div style={{ marginBottom:4 }}>3. <b>Fluoride Treatment</b> twice per year — strengthens tooth structure around restorations and prevents decay at the margins</div><div style={{ marginBottom:4 }}>4. <b>Laser Bacterial Reduction</b> at least once every 12 months — eliminates harmful bacteria below the gumline that can compromise restorations and gum health</div><div style={{ marginBottom:4 }}>5. <b>InnerView Restoration Integrity Scan</b> every 6 months — our advanced diagnostic scan detects leaking crowns, loose restorations, and hidden cracks <i>before</i> they become costly problems. Think of it as an early warning system that catches a potential $2,500+ issue when it's still a $0 fix under your warranty.</div></div>
  ...
  return (<div style={{ background:"#f0f0f0", minHeight:"100vh", fontFamily:"Arial, sans-serif" }}>
    ...
    <div className="no-print" style={{ display:"none", position:"sticky", top:0, zIndex:100, background:BLUE, padding:"10px 16px", justifyContent:"space-between", alignItems:"center" }}>
      ...
      <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
        ...
        <button onClick={() => {
          setRcptName(patientName);
          setRcptEmail(patientEmail);
          setRcptPhone(patientPhone);
          setRcptItems(treatments.filter(t => t.name).map((t, idx) => ({
            id: idx + 1,
            desc: t.name + (t.teeth.length > 0 ? " (#" + t.teeth.join(", #") + ")" : ""),
            amount: (parseFloat(t.fee) || 0).toFixed(2)
          })));
          setRcptDate(date);
          if (sameDayDiscount || inOfficePlan) setRcptDiscount(discountAmount.toFixed(2));
          if (insuranceNum > 0) setRcptInsurance(insuranceCoverage);
          setShowPreview(false);
          setAppMode("receipt");
        }} style={TB}>{"\u{1F9FE}"} Receipt</button>
    ...
    <div className="print-page">
      <div style={{ textAlign:"center", marginBottom:2 }}><Logo width={170} /></div><div style={{ borderBottom:`3px solid ${BLUE}`, marginBottom:6 }} />
      <div style={{ textAlign:"center", fontSize:20, fontWeight:700, color:BLUE, marginBottom:6 }}>Dental Treatment Plan</div>
      <div style={{ display:"flex", justifyContent:"space-between", marginBottom:3, fontSize:12 }}><div><b style={{ color:GRAY }}>Patient: </b><span style={{ borderBottom:"1px solid #ccc", display:"inline-block", minWidth:220 }}>{patientName}</span></div><div><b style={{ color:GRAY }}>Date: </b><span style={{ borderBottom:"1px solid #ccc", display:"inline-block", minWidth:130 }}>{date}</span></div></div>
      <div style={{ marginBottom:6, fontSize:12 }}><b style={{ color:GRAY }}>Treatment Needs: </b><span style={{ borderBottom:"1px solid #ccc", display:"inline-block", minWidth:350 }}>{treatmentDisplay}</span></div>
      <table style={{ width:"100%", borderCollapse:"collapse", marginBottom:5, fontSize:10.5 }}><thead><tr><th style={{ background:"#f0f0f0", padding:"3px 8px", textAlign:"left", fontWeight:700, borderBottom:"1px solid #ddd" }}>Tooth</th><th style={{ background:"#f0f0f0", padding:"3px 8px", textAlign:"left", fontWeight:700, borderBottom:"1px solid #ddd" }}>Treatment</th><th style={{ background:"#f0f0f0", padding:"3px 8px", textAlign:"right", fontWeight:700, borderBottom:"1px solid #ddd" }}>Fee</th></tr></thead><tbody>
        {treatments.filter(t=>t.name).map((t,i) => <tr key={i}><td style={{ padding:"2px 8px", borderBottom:"1px solid #eee" }}>{t.teeth.length>0?"#"+t.teeth.join(", #"):"-"}</td><td style={{ padding:"2px 8px", borderBottom:"1px solid #eee" }}>{t.name}</td><td style={{ padding:"2px 8px", borderBottom:"1px solid #eee", textAlign:"right" }}>${(parseFloat(t.fee)||0).toFixed(2)}</td></tr>)}
        <tr><td colSpan="2" style={{ padding:"3px 8px", fontWeight:700 }}>Total</td><td style={{ padding:"3px 8px", fontWeight:700, textAlign:"right" }}>${totalDebit.toFixed(2)}</td></tr>
      </tbody></table>
      <div style={{ background:LIGHT_BLUE, border:`1px solid ${BLUE}`, borderRadius:4, padding:"4px 8px", marginBottom:5, fontSize:9.5, fontStyle:"italic", color:GRAY, lineHeight:1.3 }}>* Fees may include upgraded services: {selectedUpgrades.length>0?<>{selectedUpgrades.map((s,i) => <span key={s}><b style={{ color:DARK }}>{s}</b>{i<selectedUpgrades.length-1?", ":""}</span>)}{UPGRADED_SERVICES.filter(s=>!selectedUpgrades.includes(s)).length>0&&", "}{UPGRADED_SERVICES.filter(s=>!selectedUpgrades.includes(s)).join(", ")}</>:UPGRADED_SERVICES.join(", ")}</div>
      {activeDiscount&&<div style={{ background:GREEN_BG, border:`1px solid ${GREEN}`, borderRadius:4, padding:"3px 10px", marginBottom:4, fontSize:10.5, textAlign:"center" }}><b style={{ color:GREEN }}>{discountLabel}</b><span style={{ color:DARK, marginLeft:8 }}>You save ${discountAmount.toFixed(2)}</span></div>}
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:3 }}><div><div style={{ fontSize:13, fontWeight:700 }}>Patient Pays: <span style={{ fontSize:15 }}>${creditPrice.toFixed(2)}</span></div>{insuranceNum>0&&<div style={{ fontSize:10, color:GREEN, marginTop:1 }}>Insurance: ${insuranceNum.toFixed(2)}</div>}</div><div style={{ background:LIGHT_BLUE, border:`1.5px solid ${BLUE}`, padding:"4px 10px", borderRadius:4, textAlign:"center" }}><div style={{ fontSize:9, color:BLUE }}>Debit/Cash/Check</div><div style={{ fontSize:14, fontWeight:800, color:BLUE }}>${totalDebit.toFixed(2)}</div><div style={{ fontSize:8, color:BLUE, marginTop:1 }}>Save ${savings.toFixed(2)}</div></div></div>
      {financing>0&&<div style={{ background:GOLD_BG, border:"1px solid #D4A017", borderRadius:4, padding:"3px 10px", marginBottom:4, fontSize:10.5, textAlign:"center" }}><b style={{ color:GOLD }}>{financing}mo at 0%:</b><span style={{ color:DARK, marginLeft:8 }}>${monthlyPayment.toFixed(2)}/mo</span></div>}
      <div style={{ fontSize:14, fontWeight:700, color:BLUE, marginBottom:3, marginTop:3 }}>Payment Options</div>
      <div style={{ paddingLeft:16, fontSize:10.5, lineHeight:1.5, marginBottom:4 }}><div>1. <b>Pay in full</b> at appointment</div><div>2. <b>Crowns:</b> Half at prep, half at seat</div><div>3. <b>6-month CareCredit</b> at 0%</div><div>4. <b>Cherry financing</b> as low as 0%</div></div>
      <div style={{ background:"#FFFBE6", border:"1px solid #D4A017", borderLeft:"3px solid #D4A017", borderRadius:3, padding:"3px 8px", fontSize:10, marginBottom:5 }}><b>Please note:</b> Treatment will not be completed until cost is met in full.</div>
      <div style={{ fontSize:14, fontWeight:700, color:BLUE, marginBottom:3 }}>Patient Consent & Authorization</div>
      <div style={{ fontSize:9.5, lineHeight:1.4, color:DARK }}>
        <p style={{ margin:"0 0 2px", fontWeight:700 }}>I understand if insurance does not pay I am responsible for the full amount.</p>
        <p style={{ margin:"0 0 2px" }}>Treatment options have been presented to me. I understand my insurance may not cover upgraded/cosmetic services. I release Dr. Max Buchwald Jr from the contractual terms of my plan for upgraded services.</p>
        <p style={{ margin:"0 0 2px" }}>I authorize my dental care provider to perform the listed procedures.</p>
        <p style={{ margin:"0 0 2px" }}>These fees are valid for 90 days and do not include treatment on other areas/teeth.</p>
        <p style={{ margin:"0 0 4px" }}>Cancellations with less than 48 hours notice are subject to a $100/hr charge.</p>
      </div>
      <div style={{ marginTop:6 }}><SigBlock sig={patientSig} label="Patient Signature" dateStr={date} /><SigBlock sig={coordinatorSig} label="Treatment Coordinator" dateStr={date} /></div>
    </div>
    <div className="print-page">
      <div style={{ textAlign:"center", marginBottom:2 }}><Logo width={155} /></div><div style={{ borderBottom:`3px solid ${BLUE}`, marginBottom:6 }} />
      <div style={{ textAlign:"center", fontSize:16, fontWeight:700, color:BLUE, marginBottom:5 }}>Understanding Your Dental Insurance</div>
      <p style={{ fontSize:10.5, lineHeight:1.4, margin:"0 0 4px" }}>Many patients assume medical and dental insurance are similar. They are quite different:</p>
      <table style={{ width:"100%", borderCollapse:"collapse", marginBottom:4, fontSize:10 }}><thead><tr><th style={{ background:BLUE, color:"white", padding:"4px 8px", fontWeight:700 }}>Medical Insurance</th><th style={{ background:BLUE, color:"white", padding:"4px 8px", fontWeight:700 }}>Dental Insurance</th></tr></thead><tbody>{[["High deductible","Deductibles below $100"],["High monthly premiums","Low monthly premiums"],["No annual maximum","Annual maximum applies"],["Preauthorization guarantees payment","Predetermination does not guarantee payment"]].map(([m,d],i) => <tr key={i}><td style={{ padding:"3px 8px", border:"1px solid #ddd" }}>{m}</td><td style={{ padding:"3px 8px", border:"1px solid #ddd" }}>{d}</td></tr>)}</tbody></table>
      <p style={{ fontSize:9.5, fontStyle:"italic", color:GRAY, margin:"0 0 4px", lineHeight:1.3 }}>Dental coverage is more like a discount program with a small treatment assistance benefit.</p>
      <div style={{ fontSize:13, fontWeight:700, color:BLUE, marginBottom:3 }}>Why We Prioritize Long-Lasting Work</div>
      <p style={{ fontSize:10, lineHeight:1.4, margin:"0 0 2px" }}>Many plans only cover replacement every 5 years. We use materials that last 10-15+ years:</p>
      <ul style={{ fontSize:10, lineHeight:1.5, margin:"0 0 3px", paddingLeft:16 }}><li><b>Higher-quality materials</b></li><li><b>Precise technique</b></li><li><b>Better durability</b> - fewer replacements</li></ul>
      <div style={{ borderBottom:`2px solid ${BLUE}`, marginBottom:4 }} />
      {pushWarranty ? <><div style={{ fontSize:13, fontWeight:700, color:BLUE, marginBottom:3 }}>Our Lifetime Warranty</div>
        <p style={{ fontSize:10, lineHeight:1.4, margin:"0 0 4px" }}>We offer a <b style={{ color:BLUE }}>Lifetime Warranty</b> on qualifying restorations. If it fails under normal use, we repair or replace at no charge.</p>
        <div style={{ background:"#F0FAFF", border:`1.5px solid ${BLUE}`, borderRadius:4, padding:"5px 8px", marginBottom:4, fontSize:9.5, lineHeight:1.4 }}>
          <div style={{ fontWeight:700, color:BLUE, fontSize:10, marginBottom:1 }}>Covered</div><p style={{ margin:"0 0 3px" }}>Crowns, bridges, fillings, and other restorations warranted for life.</p>
          <div style={{ fontWeight:700, color:BLUE, fontSize:10, marginBottom:1 }}>Keep It Active</div>
          <ul style={{ margin:"0 0 3px", paddingLeft:14, lineHeight:1.5 }}><li><b>Cleanings every 6 months</b></li><li><b>Night guard</b></li><li><b>Fluoride twice/year</b></li><li><b>Laser bacterial reduction every 12mo</b></li><li><b>InnerView scan every 6mo</b></li></ul>
          <div style={{ fontWeight:700, color:BLUE, fontSize:10, marginBottom:1 }}>Not Covered</div><p style={{ margin:0 }}>Trauma, accidents, failure to maintain care, neglect, or misuse.</p>
        </div></> : <div style={{ padding:"14px 0", textAlign:"center", color:GRAY, fontSize:12, fontStyle:"italic" }}>Ask us about our Lifetime Warranty program!</div>}
      <div style={{ borderBottom:`2px solid ${BLUE}`, marginBottom:4 }} />
      ...
      <div style={{ marginTop:6 }}><SigBlock sig={patientSig2} label="Patient Signature" dateStr={date} /></div>
    </div>
    </div>
  </div>);
}
