...
function db_savePatient(patient) {
  ...
  db_save("patients", patients);
  return patient.id;
}
function db_findPatient(firstName, lastName) { return db_getPatients().find(p => p.first_name.toLowerCase() === firstName.toLowerCase() && p.last_name.toLowerCase() === lastName.toLowerCase()); }
function db_getTreatments(patientId) { return db_load("treatments", []).filter(t => t.patient_id === patientId).sort((a, b) => new Date(b.created_at) - new Date(a.created_at)); }
...
function db_getNotes(patientId) { return db_load("notes", []).filter(n => n.patient_id === patientId).sort((a, b) => new Date(b.created_at) - new Date(a.created_at)); }
function db_saveNote(patientId, note) { const all = db_load("notes", []); all.unshift({ id: crypto.randomUUID(), patient_id: patientId, note, created_at: new Date().toISOString() }); db_save("notes", all); }
...
export default function TreatmentPlan() {
  ...
  const [wDate, setWDate] = useState(new Date().toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" }));
  const [wItems, setWItems] = useState([]);
  const [wCustom, setWCustom] = useState("");
  const [wChoice, setWChoice] = useState("agree");
  const [wSig, setWSig] = useState(null);
  const [wPreview, setWPreview] = useState(false);
  const [wCollectSig, setWCollectSig] = useState(false);
  // Receipt generator
  ...
  const [rcptDiscount, setRcptDiscount] = useState("");
  const [rcptInsurance, setRcptInsurance] = useState("");
  ...
  const resetReceipt = () => { setRcptName(""); setRcptEmail(""); setRcptPhone(""); setRcptItems([{id:1,desc:"",amount:""}]); setRcptPayMethod("debit"); setRcptDate(new Date().toLocaleDateString("en-US",{month:"long",day:"numeric",year:"numeric"})); setRcptDiscount(""); setRcptInsurance(""); setRcptShowPreview(false); };

  // Save patient + treatment to localStorage
  ...
  // Build email body for treatment plan
  const buildTPEmail = () => {
    let b = `Dear ${patientName},\n\nThank you for visiting Buchwald Family Dentistry! Here is your treatment plan summary.\n\nDate: ${date}\n\n`;
    b += "TREATMENT PLAN\n" + "=".repeat(40) + "\n\n";
    treatments.filter(t => t.name).forEach(t => {
      const ts = t.teeth.length > 0 ? " (Tooth #" + t.teeth.join(", #") + ")" : "";
      const p = PRIORITY_LEVELS.find(x => x.value === t.priority);
      b += `${p?.icon || ""} ${t.name}${ts}: $${(parseFloat(t.fee)||0).toFixed(2)} - Priority: ${p?.label || "Moderate"}\n`;
      b += `   Risk of delaying: ${t.customRisk || getRisk(t.name)}\n\n`;
    });
    ...
    if (financing > 0) b += `${financing} Month Payment Plan: $${monthlyPayment.toFixed(2)}/mo at 0% interest\n`;
    if (insuranceNum > 0) b += `Insurance Coverage: $${insuranceNum.toFixed(2)}\n`;
      ...
      <div style={{ minHeight:"100vh", background:"#f7f9fb", fontFamily:"'Segoe UI', system-ui, sans-serif" }}>
        ...
        <div style={{ padding:16, maxWidth:480, margin:"0 auto" }}>
          <div style={CS}><div style={SL}>Patient Info</div>
            {[["Phone", selectedPatient.phone], ["Email", pe], ["Plan", selectedPatient.plan_status === "active" ? "\u2705 Active member" : "No plan"]].map(([l,v]) => v ? <div key={l} style={{ display:"flex", justifyContent:"space-between", fontSize:13, padding:"6px 0", borderBottom:"1px solid #f0f0f0" }}><span style={{ color:GRAY }}>{l}</span><span style={{ fontWeight:600 }}>{v}</span></div> : null)}
          </div>
    ...
    return (
      <div style={{ minHeight:"100vh", background:"#f7f9fb", fontFamily:"'Segoe UI', system-ui, sans-serif" }}>
        ...
        <div style={{ background:"white", borderBottom:"1px solid #e8e8e8", display:"flex" }}>
          {[["home","\u{1F3E0}","Home"],["records","\u{1F4C1}","Patients"],["members","\u2B50","Plan"]].map(([t,i,l]) => <button key={t} onClick={() => setHubTab(t)} style={{ flex:1, padding:"12px 4px", background:"none", border:"none", borderBottom:hubTab===t?`3px solid ${BLUE}`:"3px solid transparent", color:hubTab===t?BLUE:GRAY, fontSize:12, fontWeight:hubTab===t?700:400, cursor:"pointer" }}>{i} {l}</button>)}
        </div>
        <div style={{ padding:16, maxWidth:480, margin:"0 auto" }}>
          {hubTab === "home" && (<>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10, marginBottom:16 }}>
              {[["\u2B50", members.length, "plan members"],["\u{1F4CB}", todayCount, "added today"],["\u{1F4C1}", patients.length, "total patients"]].map(([i,v,l]) => <div key={l} style={{ background:"white", borderRadius:12, padding:"14px 10px", textAlign:"center", boxShadow:"0 1px 3px rgba(0,0,0,0.07)" }}><div style={{ fontSize:20 }}>{i}</div><div style={{ fontSize:22, fontWeight:800, color:DARK, lineHeight:1.2 }}>{v}</div><div style={{ fontSize:11, color:GRAY, marginTop:2 }}>{l}</div></div>)}
            </div>
            ...
            {patients.length > 0 && (<><div style={{ fontSize:12, fontWeight:700, color:GRAY, textTransform:"uppercase", letterSpacing:1, marginBottom:10 }}>Recent Patients</div><div style={{ background:"white", borderRadius:12, boxShadow:"0 1px 3px rgba(0,0,0,0.07)", overflow:"hidden" }}>{patients.slice(0,5).map((p,i) => <div key={p.id} onClick={() => setSelectedPatient(p)} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"12px 16px", borderBottom:i<4?"1px solid #f0f0f0":"none", cursor:"pointer" }}><div><span style={{ fontSize:13, fontWeight:600, color:DARK }}>{p.first_name} {p.last_name}</span>{p.email && !p.email.includes("internal") && <span style={{ fontSize:12, color:GRAY, marginLeft:8 }}>{p.email}</span>}</div><span style={{ fontSize:11, color:GRAY }}>{timeAgo(p.created_at)}</span></div>)}</div></>)}
          </>)}

          {(hubTab === "records" || hubTab === "members") && (<>
            <div style={{ fontSize:15, fontWeight:700, color:DARK, marginBottom:12 }}>{hubTab === "members" ? "Plan Members" : "All Patients"} <span style={{ color:BLUE }}>({hubTab === "members" ? members.length : patients.length})</span></div>
            ...
            {(hubTab === "members" ? members : filtered).map(p => <div key={p.id} onClick={() => setSelectedPatient(p)} style={{ background:"white", borderRadius:12, padding:"14px 16px", marginBottom:10, boxShadow:"0 1px 3px rgba(0,0,0,0.07)", cursor:"pointer", display:"flex", justifyContent:"space-between", alignItems:"center" }}><div><div style={{ fontSize:15, fontWeight:700, color:DARK }}>{p.first_name} {p.last_name}</div>{p.phone && <div style={{ fontSize:12, color:GRAY, marginTop:2 }}>{p.phone}</div>}{p.email && !p.email.includes("internal") && <div style={{ fontSize:12, color:GRAY }}>{p.email}</div>}</div>{p.plan_status === "active" && <div style={{ background:GREEN_BG, color:GREEN, fontSize:11, fontWeight:700, padding:"4px 10px", borderRadius:20 }}>Plan</div>}</div>)}
            {(hubTab === "members" ? members : filtered).length === 0 && <div style={{ textAlign:"center", color:GRAY, padding:20, fontSize:13 }}>No patients found</div>}
          </>)}
        </div>
      </div>
    );
  ...
  if (showReceiptGen) {
      ...
      return (<div style={{ background:"#f0f0f0", minHeight:"100vh", fontFamily:"Arial, sans-serif" }}>
        ...
        <div className="no-print" style={{ display:"none", position:"sticky", top:0, zIndex:100, background:GREEN, padding:"10px 16px", justifyContent:"space-between", alignItems:"center" }}>
          ...
          <div style={{ display:"flex", gap:6 }}>
            <button onClick={() => gmailCompose(rcptEmail, `Payment Receipt - Buchwald Family Dentistry`, `Dear ${rcptName},\n\nThank you for your payment! Here is your receipt.\n\nReceipt #: ${rcptNum}\nDate: ${rcptDate}\nPayment Method: ${rcptPayMethod}\n\nItems:\n${rcptItems.filter(i=>i.desc).map(i=>`- ${i.desc}: $${(parseFloat(i.amount)||0).toFixed(2)}`).join("\n")}\n${rcptDiscNum > 0 ? `\nDiscount: -$${rcptDiscNum.toFixed(2)}` : ""}${rcptInsNum > 0 ? `\nInsurance: -$${rcptInsNum.toFixed(2)}` : ""}\n\nTotal Paid: $${rcptTotal.toFixed(2)}\n\n---\nBuchwald Family Dentistry & Orthodontics\nbuchwaldfamilydentistry.com`)} style={TB}>{"\u2709\uFE0F"} Gmail</button>
        ...
        <div id="rcpt-pdf"><div className="print-page">
          ...
          <div style={{ textAlign:"center", fontSize:11, color:GRAY, marginBottom:16 }}>Receipt #{rcptNum}</div>
          <div style={{ display:"flex", justifyContent:"space-between", marginBottom:16, fontSize:12 }}>
            <div><div style={{ color:GRAY, fontSize:10, fontWeight:700, textTransform:"uppercase", marginBottom:4 }}>Patient</div><div style={{ fontSize:14, fontWeight:700 }}>{rcptName}</div>{rcptEmail && <div style={{ fontSize:11, color:GRAY }}>{rcptEmail}</div>}{rcptPhone && <div style={{ fontSize:11, color:GRAY }}>{rcptPhone}</div>}</div>
            <div style={{ textAlign:"right" }}><div style={{ color:GRAY, fontSize:10, fontWeight:700, textTransform:"uppercase", marginBottom:4 }}>Date</div><div style={{ fontSize:14, fontWeight:700 }}>{rcptDate}</div><div style={{ fontSize:11, color:GRAY, marginTop:4 }}>Paid via {rcptPayMethod}</div></div>
          </div>
          <table style={{ width:"100%", borderCollapse:"collapse", marginBottom:16, fontSize:11.5 }}><thead><tr><th style={{ background:"#f0f0f0", padding:"6px 10px", textAlign:"left", fontWeight:700, borderBottom:"2px solid #ddd" }}>Description</th><th style={{ background:"#f0f0f0", padding:"6px 10px", textAlign:"right", fontWeight:700, borderBottom:"2px solid #ddd" }}>Amount</th></tr></thead><tbody>{rcptItems.filter(i=>i.desc).map((i,idx) => <tr key={idx}><td style={{ padding:"6px 10px", borderBottom:"1px solid #eee" }}>{i.desc}</td><td style={{ padding:"6px 10px", borderBottom:"1px solid #eee", textAlign:"right" }}>${(parseFloat(i.amount)||0).toFixed(2)}</td></tr>)}</tbody></table>
          <div style={{ borderTop:"2px solid #ddd", paddingTop:12, marginBottom:24 }}>
            {rcptDiscNum > 0 && <div style={{ display:"flex", justifyContent:"space-between", fontSize:12, marginBottom:4 }}><span style={{ color:GREEN }}>Discount</span><span style={{ color:GREEN, fontWeight:700 }}>-${rcptDiscNum.toFixed(2)}</span></div>}
            {rcptInsNum > 0 && <div style={{ display:"flex", justifyContent:"space-between", fontSize:12, marginBottom:4 }}><span>Insurance</span><span style={{ fontWeight:700 }}>-${rcptInsNum.toFixed(2)}</span></div>}
            <div style={{ display:"flex", justifyContent:"space-between", fontSize:16, fontWeight:800, color:BLUE, borderTop:"2px solid #ddd", paddingTop:8, marginTop:8 }}><span>Total Paid</span><span>${rcptTotal.toFixed(2)}</span></div>
          </div>
    ...
    return (<div style={{ minHeight:"100vh", background:"#f7f9fb", fontFamily:"'Segoe UI', system-ui, sans-serif" }}>
      <div style={{ background:GREEN, padding:"14px 20px", display:"flex", alignItems:"center", gap:12 }}><button onClick={() => { resetReceipt(); setShowReceiptGen(false); }} style={TB}>{"\u2190"} Back</button><div style={{ color:"white", fontSize:16, fontWeight:700 }}>{"\u{1F9FE}"} Receipt Generator</div></div>
      <div style={{ padding:"20px 16px", maxWidth:480, margin:"0 auto" }}>
        <div style={CS}><div style={SL}>Patient Info</div>
          ...
          <label style={LS}>Email <span style={{ fontWeight:400, color:"#999" }}>(for Gmail receipt)</span></label><input type="email" value={rcptEmail} onChange={e => setRcptEmail(e.target.value)} placeholder="patient@email.com" style={IS} />
        ...
        <div style={CS}><div style={SL}>Payment Details</div>
          ...
          {rcptSubtotal > 0 && <div style={{ marginTop:16, background:"#f7f9fb", borderRadius:10, padding:16 }}>
            ...
            <div style={{ display:"flex", justifyContent:"space-between", fontSize:16, fontWeight:800, color:BLUE, background:LIGHT_BLUE, margin:"6px -8px 0", padding:"10px 8px", borderRadius:"0 0 8px 8px" }}><span>Total</span><span>${rcptTotal.toFixed(2)}</span></div>
          </div>}
        </div>
    ...
    return (<div style={{ background:"#f0f0f0", minHeight:"100vh", fontFamily:"Arial, sans-serif" }}>
      ...
      <div className="no-print" style={{ display:"none", position:"sticky", top:0, zIndex:100, background:BLUE, padding:"10px 16px", justifyContent:"space-between", alignItems:"center" }}>
        ...
        <div style={{ display:"flex", gap:6 }}>
          ...
          <button onClick={() => gmailCompose("", `Lifetime Warranty - ${wName} - Buchwald Family Dentistry`, `Lifetime Warranty Form\n\nPatient: ${wName}\nDate: ${wDate}\n\nTreatments:\n${wItems.map(i=>`- ${i.name}${i.teeth.length>0?" (#"+i.teeth.join(", #")+")":""}`).join("\n")}\n\nElection: ${wChoice==="agree"?"Agreed":"Waived"}\n\n---\nBuchwald Family Dentistry & Orthodontics`)} style={TB}>{"\u2709\uFE0F"} Gmail</button>
      ...
      <div id="w-pdf"><div className="print-page">
        ...
        <div style={{ background:"#F9FBF2", border:"1.5px solid #8AAE2B", borderRadius:4, padding:"6px 10px", marginBottom:8, fontSize:10 }}><div style={{ fontWeight:700, color:"#5A7A10", fontSize:11, marginBottom:4 }}>What This Saves You</div><div style={{ display:"flex", justifyContent:"space-between", marginBottom:2 }}><span>Crown replacement:</span><span style={{ fontWeight:700 }}>$2,500\u2013$3,000</span></div><div style={{ display:"flex", justifyContent:"space-between", marginBottom:2 }}><span>Implant crown replacement:</span><span style={{ fontWeight:700 }}>$4,000\u2013$6,000</span></div><div style={{ display:"flex", justifyContent:"space-between", borderTop:"1px dashed #8AAE2B", paddingTop:3, marginTop:2 }}><span style={{ fontWeight:700, color:"#5A7A10" }}>With Lifetime Warranty:</span><span style={{ fontWeight:700, color:"#5A7A10" }}>$0</span></div></div>
        ...
        <div style={{ paddingLeft:14, fontSize:10, lineHeight:1.65, marginBottom:6 }}>
          <div style={{ marginBottom:2 }}>1. <b>Regular Cleanings</b> every 6-7 months</div><div style={{ marginBottom:2 }}>2. <b>Custom Nightguard</b> (starting at $400 depending on insurance)</div><div style={{ marginBottom:2 }}>3. <b>Fluoride Treatment</b> twice per year</div><div style={{ marginBottom:2 }}>4. <b>Laser Bacterial Reduction</b> every 12 months</div><div style={{ marginBottom:2 }}>5. <b>InnerView Scan</b> every 6 months</div>
        </div>
        <p style={{ fontSize:10, fontWeight:700, color:DARK, marginBottom:6, background:GOLD_BG, border:"1px solid #D4A017", borderRadius:3, padding:"4px 8px" }}>If any of these requirements are not maintained, warranty coverage will be voided.</p>
        <div style={{ fontSize:10, marginBottom:2, color:GRAY }}><b>Warranty Effective Date:</b> Coverage begins <b>5 years</b> from signature date.</div>
        ...
        <div style={{ marginBottom:8, fontSize:10.5, lineHeight:1.65 }}>
          ...
          <div style={{ display:"flex", alignItems:"flex-start", gap:8 }}><div style={{ width:45, borderBottom:"1.5px solid #999", flexShrink:0, marginTop:8, textAlign:"center" }}>{wChoice==="waive"&&<span style={{ fontSize:15, fontWeight:700 }}>{"\u2713"}</span>}</div><span><b>I elect to waive</b> the warranty and release Buchwald Family Dentistry from obligation to replace or repair.</span></div>
        </div>
  ...
  if (showReceipt) {
    ...
    return (<div style={{ background:"#f0f0f0", minHeight:"100vh", fontFamily:"Arial, sans-serif" }}>
      ...
      <div id="tp-rcpt"><div className="print-page">
        <div style={{ textAlign:"center", marginBottom:6 }}><Logo width={170} /></div><div style={{ borderBottom:`3px solid ${BLUE}`, marginBottom:12 }} />
        <div style={{ textAlign:"center", fontSize:22, fontWeight:700, color:BLUE, marginBottom:4 }}>Payment Receipt</div><div style={{ textAlign:"center", fontSize:11, color:GRAY, marginBottom:16 }}>#{rn}</div>
        <div style={{ display:"flex", justifyContent:"space-between", marginBottom:16, fontSize:12 }}><div><div style={{ color:GRAY, fontSize:10, fontWeight:700, textTransform:"uppercase", marginBottom:4 }}>Patient</div><div style={{ fontSize:14, fontWeight:700 }}>{patientName}</div>{patientEmail&&<div style={{ fontSize:11, color:GRAY }}>{patientEmail}</div>}</div><div style={{ textAlign:"right" }}><div style={{ color:GRAY, fontSize:10, fontWeight:700, textTransform:"uppercase", marginBottom:4 }}>Date</div><div style={{ fontSize:14, fontWeight:700 }}>{date}</div><div style={{ fontSize:11, color:GRAY, marginTop:4 }}>Paid via {paymentMethod}</div></div></div>
        <table style={{ width:"100%", borderCollapse:"collapse", marginBottom:16, fontSize:11.5 }}><thead><tr><th style={{ background:"#f0f0f0", padding:"6px 10px", textAlign:"left", fontWeight:700, borderBottom:"2px solid #ddd" }}>Treatment</th><th style={{ background:"#f0f0f0", padding:"6px 10px", textAlign:"right", fontWeight:700, borderBottom:"2px solid #ddd" }}>Amount</th></tr></thead><tbody>{treatments.filter(t=>t.name).map((t,i) => <tr key={i}><td style={{ padding:"6px 10px", borderBottom:"1px solid #eee" }}>{t.name}{t.teeth.length>0?" (#"+t.teeth.join(", #")+")":""}</td><td style={{ padding:"6px 10px", borderBottom:"1px solid #eee", textAlign:"right" }}>${(parseFloat(t.fee)||0).toFixed(2)}</td></tr>)}</tbody></table>
        <div style={{ borderTop:"2px solid #ddd", paddingTop:12, marginBottom:24 }}>
          ...
          {insuranceNum>0&&<div style={{ display:"flex", justifyContent:"space-between", fontSize:12, marginBottom:4 }}><span>Insurance</span><span style={{ fontWeight:700 }}>-${insuranceNum.toFixed(2)}</span></div>}
        ...
        <div style={{ background:GREEN_BG, border:`2px solid ${GREEN}`, borderRadius:8, padding:"16px 20px", textAlign:"center", marginBottom:24 }}><div style={{ fontSize:16, fontWeight:700, color:GREEN }}>{"\u2713"} PAID IN FULL</div></div>
        <div style={{ textAlign:"center", fontSize:10, color:GRAY, lineHeight:1.6 }}><div>Buchwald Family Dentistry & Orthodontics</div><div>buchwaldfamilydentistry.com</div></div>
      </div></div>
    </div>);
  }
    ...
    return (<div style={{ minHeight:"100vh", background:"#f7f9fb", fontFamily:"'Segoe UI', system-ui, sans-serif" }}>
      ...
      <div style={{ padding:"20px 16px", maxWidth:480, margin:"0 auto" }}>
        <div style={CS}><div style={SL}>Patient Info</div>
          ...
          <label style={LS}>Email <span style={{ fontWeight:400, color:"#999" }}>(for Gmail copy)</span></label><input type="email" value={patientEmail} onChange={e => setPatientEmail(e.target.value)} placeholder="patient@email.com" style={IS} />
        ...
        <div style={CS}><div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}><div style={SL}>Treatments</div><button onClick={addTreatment} style={{ background:LIGHT_BLUE, color:BLUE, border:`1.5px solid ${BLUE}`, borderRadius:8, padding:"6px 12px", fontSize:13, fontWeight:700, cursor:"pointer" }}>+ Add</button></div>
          {treatments.map((t, idx) => <div key={t.id} style={{ background:"#f7f9fb", borderRadius:10, padding:"12px 14px", marginBottom:10, position:"relative" }}>
            ...
            <label style={{ ...LS, marginTop:0, marginBottom:8 }}>Priority</label>
            <div style={{ display:"flex", gap:6, marginBottom:10 }}>{PRIORITY_LEVELS.map(p => <button key={p.value} onClick={() => updateTreatment(t.id,"priority",p.value)} style={{ flex:1, padding:"6px 4px", borderRadius:8, border:`1.5px solid ${t.priority===p.value?p.color:"#ddd"}`, background:t.priority===p.value?p.bg:"white", color:t.priority===p.value?p.color:GRAY, fontSize:11, fontWeight:t.priority===p.value?700:400, cursor:"pointer", textAlign:"center" }}>{p.icon} {p.label}</button>)}</div>
            {t.name && <div style={{ marginBottom:10 }}><label style={{ ...LS, marginTop:0, marginBottom:4 }}>{"\u26A0\uFE0F"} Risk of Delaying <span style={{ fontWeight:400, color:"#999", fontSize:10 }}>(shown to patient)</span></label><textarea value={t.customRisk || getRisk(t.name)} onChange={e => updateTreatment(t.id,"customRisk",e.target.value)} style={{ width:"100%", padding:"10px 12px", border:"1.5px solid #e0e0e0", borderRadius:10, fontSize:12, resize:"vertical", minHeight:60, boxSizing:"border-box", background:"#fafafa", lineHeight:1.5 }} /></div>}
            <label style={{ ...LS, marginTop:0, marginBottom:8 }}>Tooth # <span style={{ fontWeight:400, color:"#999" }}>(optional)</span></label>
  ...
  return (<div style={{ background:"#f0f0f0", minHeight:"100vh", fontFamily:"Arial, sans-serif" }}>
    ...
    <div className="no-print" style={{ display:"none", position:"sticky", top:0, zIndex:100, background:BLUE, padding:"10px 16px", justifyContent:"space-between", alignItems:"center" }}>
      ...
      <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
        ...
        <button onClick={() => { gmailCompose(patientEmail, "Your Treatment Plan - Buchwald Family Dentistry", buildTPEmail()); setEmailSent(true); }} style={{ ...TB, background:emailSent?"rgba(45,138,78,0.5)":"rgba(255,255,255,0.2)" }}>{emailSent?"\u2713 Sent":"\u2709\uFE0F Gmail"}</button>
    ...
    <div className="print-page">
      ...
      <div style={{ fontSize:9.5, lineHeight:1.5, color:DARK }}>
        <p style={{ margin:"0 0 3px", fontWeight:700 }}>I understand if insurance does not pay for any reason that I am responsible for the full amount.</p>
        <p style={{ margin:"0 0 3px" }}>Treatment options have been presented to me. I understand my insurance may not cover upgraded/cosmetic services and I am responsible for those fees. I release Dr. Max Buchwald Jr from the contractual terms of my plan for upgraded services.</p>
        <p style={{ margin:"0 0 3px" }}>I authorize my dental care provider to perform the listed procedures. I have discussed treatment options and received details and post-op instructions.</p>
        <p style={{ margin:"0 0 3px" }}>I acknowledge these fees are valid for 90 days and do not include treatment on other areas/teeth.</p>
        <p style={{ margin:"0 0 5px" }}>Appointments cancelled with less than 48 hours notice are subject to a $100/hr charge.</p>
      </div>
    ...
    <div className="print-page">
      ...
      <p style={{ fontSize:10.5, lineHeight:1.5, margin:"0 0 3px" }}>Many plans only cover replacement every 5 years. We use techniques and materials that last 10-15+ years:</p>
      <ul style={{ fontSize:10.5, lineHeight:1.6, margin:"0 0 3px", paddingLeft:18 }}><li><b>Higher-quality materials</b></li><li><b>Precise technique</b> and attention to detail</li><li><b>Better durability</b> \u2014 fewer replacements, better value</li></ul>
      ...
      {pushWarranty ? <><div style={{ fontSize:14, fontWeight:700, color:BLUE, marginBottom:4 }}>Our Lifetime Warranty</div>
        <p style={{ fontSize:10.5, lineHeight:1.5, margin:"0 0 5px" }}>Buchwald Family Dentistry offers a <b style={{ color:BLUE }}>Lifetime Warranty</b> on qualifying restorations. If a warranted restoration fails under normal use, we repair or replace at no charge.</p>
        <div style={{ background:"#F0FAFF", border:`1.5px solid ${BLUE}`, borderRadius:4, padding:"7px 10px", marginBottom:6, fontSize:10, lineHeight:1.5 }}>
          <div style={{ fontWeight:700, color:BLUE, fontSize:11, marginBottom:2 }}>What is Covered</div><p style={{ margin:"0 0 5px" }}>Crowns, bridges, fillings, and other restorations warranted for life against defects.</p>
          <div style={{ fontWeight:700, color:BLUE, fontSize:11, marginBottom:2 }}>Keep Your Warranty Active</div>
          <ul style={{ margin:"0 0 5px", paddingLeft:16, lineHeight:1.6 }}><li><b>Cleanings every 6 months</b></li><li><b>Night guard</b> \u2014 protects from grinding</li><li><b>Fluoride twice/year</b></li><li><b>Laser bacterial reduction every 12 months</b></li><li><b>InnerView scan every 6 months</b></li></ul>
